...
export default function SmsThread() {
  ...
  return (
    <div className="flex flex-col h-dvh">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 bg-card border-b border-hairline flex-shrink-0">
        ...
        <div>
          <p className="font-semibold text-ink">{conversation.customer_name || conversation.phone_number || 'Conversation'}</p>
          {conversation.phone_number && <p className="text-xs text-muted">{conversation.phone_number}</p>}
        </div>
      </div>
        ...
        {messages.map((msg, i) => {
          ...
          return (
            <div key={msg.id || i} className={`flex ${isOutbound ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${isOutbound ? 'bg-blue text-white rounded-br-sm' : 'bg-card text-ink shadow rounded-bl-sm'}`}>
                <p className="text-sm">{msg.body || msg.message}</p>
                <p className={`text-[10px] mt-1 ${isOutbound ? 'text-blue-200' : 'text-muted'}`}>{formatTime(msg.created_at || msg.date)}</p>
              </div>
            </div>
          );
        })}
      ...
      {/* Input */}
      <form onSubmit={handleSend} className="flex-shrink-0 flex items-center gap-3 px-4 py-3 bg-card border-t border-hairline" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 12px)' }}>
        <input
          type="text"
          value={body}
