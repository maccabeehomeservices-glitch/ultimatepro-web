...
export default function JobTypes() {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-10">
      ...
      {/* Trades */}
      <Card className="mb-4">
        <p className="text-sm font-semibold text-ink mb-1">Your trades</p>
        <p className="text-xs text-muted mb-3">Pick the trades you do — each suggests common job types below.</p>
          ...
          {Object.entries(registry).map(([key, t]) => (
            <button key={key} type="button" onClick={() => toggleTrade(key)} disabled={saving}
              className={`px-3 py-1.5 rounded-full text-sm font-medium border min-h-[36px] transition-colors ${
                selected.includes(key) ? 'bg-blue text-white border-blue' : 'bg-card text-ink border-hairline hover:bg-background'
              }`}>{t.label}</button>
          ))}
        </div>
      </Card>
      ...
      {selected.length > 0 && (
        <Card className="mb-4">
          <p className="text-sm font-semibold text-ink mb-1">Suggested job types</p>
          <p className="text-xs text-muted mb-3">Tap to add. Already-added types are marked.</p>
          {selected.map((tradeKey) => (
            <div key={tradeKey} className="mb-3">
              <p className="text-xs font-semibold text-muted uppercase mb-1.5">{registry[tradeKey]?.label}</p>
              <div className="flex flex-wrap gap-2">
      ...
      <Card>
        <p className="text-sm font-semibold text-ink mb-1">Your job types</p>
        <p className="text-xs text-muted mb-3">These are the chips shown when creating a job.</p>
        <div className="flex flex-wrap gap-2 mb-4">
          {active.map((t) => (
            <span key={t.id} className="px-3 py-1.5 rounded-full text-sm font-medium bg-background text-ink border border-hairline flex items-center gap-1.5 min-h-[36px]">
              {t.label}
              <button type="button" onClick={() => removeType(t.id)} className="text-muted hover:text-red-500" aria-label={`Remove ${t.label}`}><X size={14} /></button>
            </span>
          ))}
          {active.length === 0 && <span className="text-sm text-muted">No job types yet — add one below.</span>}
        </div>
