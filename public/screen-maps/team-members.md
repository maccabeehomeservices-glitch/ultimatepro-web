...
| `last_verified` | 2026-06-07 · Granular permissions PHASE 1 (FOUNDATION, NO ENFORCEMENT): `users.permissions` JSONB added (null = role template); `backend/utils/permissions.js` source of truth (9 sections × 4 levels + role templates + `hasPermission`/`resolvePermissions`/`validatePermissions`); `GET /users/permission-schema`; permission grid on Add/Edit (both platforms); "Add/Edit Team Member" rename. **Phases 2a/2b/2c (DONE): `requirePermission` + own-scope middleware enforce all 9 sections server-side; the seam (`canApproveEarnings`) delegates to `hasPermission`. Phase 3a-0: login/register + /me return `permissions_resolved`; web `can()` + android `canPermission()` helpers. Phase 3a (web + android, DONE): controls + nav hidden client-side per permission to mirror the server. Web Sidebar (Payments/Reports/Payroll) + Android "More"/Settings entries (Reports/Payroll/Payments/Job Sources/Review Platforms/Team Members/Integrations/Technicians) hidden when the section level is below `view`. Write controls gated both platforms: Add Team Member = team_settings:full; Add Customer / Charge Payment / Collect Deposit = customers/payments edit_self; Add pricebook item = pricebook:edit_self, edit/delete = full; payroll Mark-Paid = accounting_earnings:full; source/review writes = respective full; Delete job/estimate = jobs/est_inv:full. Creates (New Job, estimate/invoice) NOT hidden (all roles ≥ edit_self). Driven by `permissions_resolved` (login/register/me) via web `can()` / android `canPermission()`.** Prior: 2026-06-06 Tier 2.1 web fix. |
