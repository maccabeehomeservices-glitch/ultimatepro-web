...
- **end_state:** Job edit form open.
- **failure_modes:** none
- **parity:** MATCH. Assigned shows the assigned user (`tech_first`/`tech_last`) XOR the roster tech (flat `roster_tech_name`), "Unassigned" when both null — both platforms (P2.1b added `roster_tech_name` to the Android `Job` model + an "Unassigned" fallback; roster-tech assignments no longer render blank on the detail). Source shows `job_source_name ?: ad_channel_name ?: "My Company"` on both platforms (P2.1c fixed the Android own-company case, which previously rendered "—" blank).
- **status:** OK
- **status_note:** n/a
