...
- **status:** OK
- **status_note:** Timezone resolved on both platforms (TZ 1/3+2/3+3/3). Each job carries its address timezone (`jobs.job_timezone`, resolved from lat/lng via `tz-lookup`); the backend is the single converter (luxon: `scheduled_local` + zone → UTC `TIMESTAMPTZ`); responses expose `effective_timezone`. Web uses date-fns-tz, Android uses java.time (native on minSdk 26). **Display-fix correction (device-testing follow-up):** the 2/3 + 3/3 notes claimed "both surfaces display in effective_timezone" but the **Jobs LIST rows were a missed surface**, web `Jobs.jsx`/`humanizeScheduled` rendered browser-local and Android `JobListScreen`/`humanizeScheduled` mis-parsed the `.000Z` to device-local. Now fixed: **all four read surfaces, list, calendar, detail, edit-prefill, render in the job zone with a zone label** on both platforms. Additionally **both date-only Joby `job_assigned` auto-messages**, the tech "New Job Alert" and the customer "Booking Confirmation" (SMS + email), now include the time (`{{job_time}}`) in the job's zone (were date-only); seed updated + the one-time `scripts/backfill-joby-jobtime.js` migrated existing companies' templates (14 rows: 7 tech + 7 customer). The third `job_assigned` customer message, the **"24h Reminder"**, already carried `{{job_time}}` (no `{{job_date}}`) and likewise renders in the job's zone via the same shared `buildVars` change, no migration needed. So **all three** `job_assigned` messages now show job-zone time. Backfill `scripts/backfill-job-timezones.js` (already run in prod: 63 jobs → America/New_York) populates pre-existing jobs.
...
- **side_effects:** none
- **end_state:** n/a.
- **failure_modes:** none, the dead `<select>` + both `form.status` seeds were deleted (JobForm.jsx).
- **parity:** n/a (gone from web; Android never had it).
- **status:** REMOVED
- **status_note:** 2026-06-01 dead-code removal: the edit-mode Status `<select>` and the `form.status` seeds were deleted from JobForm.jsx. Status changes only via `POST /jobs/:id/status` from Job Detail.
...
- **Web Status field RESOLVED (2026-06-01):** the dead edit-mode Status `<select>` + `form.status` seeds were removed from JobForm.jsx. Status is set only from Job Detail.
