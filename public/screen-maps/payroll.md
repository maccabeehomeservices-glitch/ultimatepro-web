...
| `route_android` | `payroll` → `PayrollScreen` (PayrollScreens.kt) + `ActorReportScreen` (per-actor drill, route `payroll/report/{actorType}/{id}`), `TechPaySettingsScreen`, `ReimbursementsScreen`, `ProfitSimulatorScreen` |
...
| `last_verified` | 2026-07-08 · P2.27 step 4b: Android per-actor drill migrated off `/payroll/tech-report` onto the Bundle-4 `/reports/{tech,roster,source}/:id` endpoints (same reference columns + numbers as web). New `ActorReportScreen` (Pay Statement / Source Settlement, reference-column rows + payment-method breakdown); Overview + By-Tech route by `actorType`; By-Tech now includes roster + sources (F-b). Old `TechReportScreen`/`getTechReport` removed. VERIFIED ON-DEVICE (emulator up_test): Rob Roster month = 2 jobs/$1,050/$420 net/$780 all-time reconciles to Overview + web; Google Local Services source = $75/1 job; dates formatted (F-c). Artifacts scratchpad/p27b_*.png. Prior: 2026-07-06 · P2.1h/F9: web `/payroll` now route-gated by `RequirePermission section=accounting_earnings level=view` (mirrors the backend `/payroll/summary` guard) — a role with `accounting_earnings:none` (e.g. dispatcher) gets a clean "Access denied" instead of the shell; a technician (`edit_self`) still passes, matching the backend which serves them the view. Sidebar + mobile BottomNav hide Payroll via `can(accounting_earnings,view)`. Prior: 2026-07-06 · P2.1f/F7: fixed the Android `/payroll/summary` ROSTER branch job-count fan-out (was a jobs×tech_earnings cross-join inflating roster earnings ~8×) — rewritten to LATERAL like the user branch; contract test asserts roster earnings == Σ per-job tech_profit == /reports/earnings. Prior: 2026-06-06 · Option-1 payroll: dead period-lock/finalize machinery RETIRED (4 routes + 4 Android bindings removed); range-based mark-paid added (`POST /payroll/earnings/mark-paid`, reuses `tech_earnings.paid/paid_at`, no periods); `saveJobEarnings` now preserves `paid/paid_at` across recompute. `payroll_periods` table + `payroll_period_id` columns retained-but-inert (no migration). Prior: 2026-05-31 Stage-1 audit, commit 79940c8. |

### load_sequence
**Web:** `GET /reports/earnings?from=&to=` (+ `/users/technicians`, `/roster-techs`, `/sources/contacts` to resolve drill-down ids by name). **Android:** `vm.loadSummary(period)` → `GET /payroll/summary` (period = today/week/month/custom); Job-Report tab → `GET /payroll/job-report`; per-actor drill (`ActorReportScreen`) → `GET /reports/{tech,roster,source}/:id?from=&to=` (P2.27 step 4b).
...
- **precondition:** n/a
- **confirm:** n/a
- **route_chain:** web → `/reports/team/:userId` | `/reports/roster/:id` | `/reports/source/:id` (ids resolved by name from the directory endpoints); Android "Full Report" / By-Tech card → `ActorReportScreen` which calls the SAME `GET /reports/{tech,roster,source}/:id` endpoints as web (P2.27 step 4b — migrated off the old `/payroll/tech-report`), routing by `TechPaySummary.actorType`. Android sends explicit `from`/`to` (month/week-to-date) so the drill reconciles to the cent with the Overview cards + web.
- **request_body:** n/a
- **side_effects:** `navigate`.
- **end_state:** Actor report screen. tech/roster = **"Tech Balance Sheet"** (P2.27, all 3 surfaces — web `TechBalanceSheet.jsx`, Android `TechBalanceRow`, report PDF `drawBalanceSheetTable`): per-job columns Ticket # | Client Info (name+addr) | Date Closed | Job Type | Total | Payment ("Method (Collector)", one line per payment; split=multi-line; method map cc/scanpay/venmo/cashapp/payment_link→CC, zelle/check→Check, cash→Cash, other→Other) | Parts & Fees (T.Part/C.Part/Fees) | Tech Profit (Cut + pay_model-adaptive Rate/Hours) | Balance (per-job settlement = tech_collected − cut − tech-paid parts; +ve tech owes company, −ve company owes tech). Bottom TOTAL + REPORT BALANCE (no top headline / no all-time line). source = Source Settlement (unchanged).
- **failure_modes:** web drill-down silently disabled when the name→id lookup misses (no `targetPath`).
- **parity:** CONVERGED (P2.27 step 4b) — both platforms now drill into the SAME `/reports/{actor}` endpoints with the same numbers; Android's By-Tech tab now lists roster techs + sources (sourced from `payroll/summary`, F-b fixed), previously user-techs only.
- **status:** OK
...
- **UNVERIFIED:** Android `tech-settings` PUT body, `simulate` body. (Android `By Tech` tab VERIFIED 2026-07-08 P2.27 4b: lists all `payroll/summary` actors incl. roster/source, drills via `actorType` to `ActorReportScreen`.)
